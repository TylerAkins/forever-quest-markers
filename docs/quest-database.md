...
This document is the index of list-page URLs. The same list lives in `tools/quest_db/sources.py` and `data/forever-quests/manifest.json`.

**Last checked for changes:** see `data/forever-quests/manifest.json` → `lastCheckedForChanges`.
...
Addon cutover is **not** part of this work; see `docs/QUEST_DATABASE_CUTOVER.md`.
...
Full values: `data/forever-quests/pin_categories.json`.
