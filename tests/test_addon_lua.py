...
class AddonLuaTests(unittest.TestCase):
    ...
    def test_auto_quest_options(self) -> None:
        ...
        self.assertIn("dirtyOptions", config)
        self.assertIn("AttachSettings(false)", config)
        ...
        self.assertIn('self:SetScript("OnClick", nil)', config)
        self.assertNotIn("if applying or not settingsReady then", config)
        ...
        self.assertIn("AttachSettings(false)", hydrate)
        self.assertNotIn("BindSavedTable", hydrate)
        ...
        self.assertIn("ns.SyncSettingsCheckboxes()", pew)
