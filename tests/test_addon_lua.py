...
class AddonLuaTests(unittest.TestCase):
    ...
    def test_eligibility_function_exists(self) -> None:
        ...
        self.assertNotIn("IsQuestFlaggedCompletedOnAccount", text)
        list_wins = text.find("if completedListLoaded then")
        single_id = text.find('Call(C_QuestLog, "IsQuestFlaggedCompleted"', list_wins)
        self.assertNotEqual(list_wins, -1)
        self.assertNotEqual(single_id, -1)
        self.assertLess(list_wins, single_id)
        self.assertIn("return lookup[questID] and true or false", text)
    ...
    def test_object_started_prereqs_and_morin_patrol(self) -> None:
        ...
        self.assertIn("[749] = { mapID=1412, x=53, y=60.2, qg=2988", db)
        self.assertIn("{ 53.8, 48.3, 1412 }", db)
        self.assertNotRegex(db, r"\[751\] = \{[^}]*qg=")
        self.assertIn("[764] = { mapID=1412, x=53, y=60.2, qg=2988", db)
